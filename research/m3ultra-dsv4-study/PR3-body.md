...
| LLQ head-loop double buffer | stage tile t+1 while `simdgroup_multiply_accumulate`-ing tile t | the deep-context decode/TTFT gains above |
...
```
decode @117k ctx:   32.47 → 35.76 t/s       +3.9%   (medians, non-overlaping)
...
Compared across factory `main`, kernel-subset, host-subset, and full branch tips at multiple prompt sizes (96 / 512 / 2048 / 5000-word): `nan=0 inf=0` everywhere, deterministic per build, no all-scan anomalies. Several genuinely-negative candidates were found on the way (not in this PR): a Metal-runtime-arg toggle family that produced broken shader-states (archived at `toggle-attempts-ref` in `daemon2k3/ds4` for the record, plus bisect notes), and an f16 indexer-compressed-K cache (`indexer-comp-f16` — nondeterministic at depth, parked).
...
Branch: `daemon2k3/ds4:code-layer-wins` (tip `594757c`). Recommended reading order: the big host-side commit, then the kernel commit, then the zero-copy/C1 commits. Deprecated/env-gated rollback maintains one-flag-per-change convention where applicable (`DS4_DISABLE_ZEROCOPY_PAYLOAD_SPANS`, and the C1 flag is itself opt-in).
